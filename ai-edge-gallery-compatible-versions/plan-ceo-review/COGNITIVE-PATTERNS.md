...
When evaluating architecture, think through the **inversion reflex**. When challenging scope, apply **focus as subtraction**. When assessing timeline, use **speed calibration**. When probing whether the plan solves a real problem, activate **proxy skepticism**. When evaluating UI flows, apply **hierarchy as service** and **subtraction default**. When reviewing user-facing features, activate **design for trust** and **edge case paranoia**.
